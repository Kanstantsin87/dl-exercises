...
class LinearRegression():
    ...
    def normalization(x): 
        ...
        ret = []
        def zero_div(v, d):
            return v / d if d != 0 else 0

        for col in x.T:
            avg = np.average(col)
            max_min = np.max(col) - np.min(col)
            ret.append([(lambda d: zero_div((d - avg), max_min))(i) for i in col])
    ...
    def cost(self, x, y):
        ...
        return np.square(y_pred - y).mean() / 2
    ...
    def __gradient(self, x, y):
        ...
        y_pred = self.predict(x)
        return ((y_pred - y).dot(x)) / len(y_pred)
